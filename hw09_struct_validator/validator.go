...
func Validate(v interface{}) error {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if errors.Is(err, ErrTagInvalid) || errors.Is(err, ErrTypeUnsupported) {
			return err
		}
		if err != nil {
...
func validateField(field reflect.StructField, value reflect.Value) *ValidationError {
	...
	for tagType, tagVal := range tags {
		...
		case tagType == regexpTag:
			v, err := regexp.Compile(tagVal)
			if err != nil {
				return &ValidationError{Field: field.Name, Err: ErrTagInvalid}
			}
			err = validateRegexp(value, v)
			if err != nil {
				return &ValidationError{Field: field.Name, Err: err}
			}
		case tagType == minTag:
...
func validateRegexp(value reflect.Value, tag *regexp.Regexp) error {
	switch value.Kind() { //nolint:exhaustive
	case reflect.String:
	...
	case reflect.Slice:
		for i := 0; i < value.Len(); i++ {
			err := validateRegexp(value.Index(i), tag)
			if err != nil {
				return err
			}
		}
